...
class PDF extends FPDF
{
    ...
    function Header()
    {
        ...
        $this->Cell(50, 1, 'Form  EDEN ESTATE To EDEN ESTATE', 0, 1, 'L');
    ...
    function CreateBody()
    {
        $colWidths = [60, 50, 20, 60, 40, 40, 20];
        $this->SetFillColor(192, 192, 192);
        $this->SetFont('Arial', '', 10);

        $this->Cell(10);
        $this->Cell($colWidths[0], 5, 'Code-Earning', 'B', 0, 'L');
        $this->Cell($colWidths[1], 5, 'Deduction', 'B', 0, 'L');
        $this->Cell($colWidths[2], 5, 'No', 'B', 0, 'L');
        $this->Cell($colWidths[3], 5, 'Amount', 'B', 0, 'L');
        $this->Cell($colWidths[4], 5, 'Co. Contrib', 'B', 0, 'L');
        $this->Cell($colWidths[5], 5, 'Additional Infor', 'B', 1, 'L');
        ...
        foreach ($users as $user) {
            ...
            $this->Cell($colWidths[2], 5, $user['client_id'], '', 0, 'L');
            $this->Cell($colWidths[3], 5, number_format($invoiceTotal, 2), '', 0, 'L');
            $this->Cell($colWidths[4], 5, number_format($invoiceTotal, 2), '', 0, 'L');
            $this->Cell($colWidths[5], 5, number_format($invoiceTotal, 2) . " TOT", '', 1, 'L');
        }

        $this->Ln(10);
        $this->Cell(10);
        $this->Cell($colWidths[0], 10, '', '', 0, 'L');
        $this->Cell($colWidths[1], 10, '', 'B', 0, 'L');
        $this->Cell($colWidths[2], 10, 'Totals', 'B', 1, 'L');

        $this->Cell(10);
        $this->Cell($colWidths[0], 10, '', '', 0, 'L');
        $this->Cell($colWidths[1], 10, 'EARNINGS', '', 0, 'L');
        $this->Cell($colWidths[2], 10,  number_format($grandTotal, 2), '', 1, 'L');
        $this->Cell(10);
        $this->Cell($colWidths[0], 10, '', '', 0, 'L');
        $this->Cell($colWidths[1], 10, 'DEDUCTIONS', '', 0, 'L');
        $this->Cell($colWidths[2], 10,  number_format($grandTotal, 2), '', 1, 'L');
        $this->Cell(10);
        $this->Cell($colWidths[0], 10, '', '', 0, 'L');
        $this->Cell($colWidths[1], 10, 'NET PAY', '', 0, 'L');
        $this->Cell($colWidths[2], 10,  number_format($grandTotal, 2), '', 1, 'L');
        $this->Cell(10);
        $this->Cell($colWidths[0], 10, '', '', 0, 'L');
        $this->Cell($colWidths[1], 10, 'BENEFITS', '', 0, 'L');
        $this->Cell($colWidths[2], 10,  '', '', 1, 'L');
        $this->Cell(10);
        $this->Cell($colWidths[0], 10, '', '', 0, 'L');
        $this->Cell($colWidths[1], 10, 'GROSS COSTS', 'B', 0, 'L');
        $this->Cell($colWidths[2], 10,  number_format($grandTotal, 2), 'B', 1, 'L');
    }
